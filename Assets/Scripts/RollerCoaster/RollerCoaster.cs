...
public class RollerCoaster : MonoBehaviour {
	...
	void Update () {
        CreateInclinePath(Vector3.zero, currentTrack);
    ...
    public void CreateTurningPath(Vector3 position, GameObject startTrack) {
    ...
    //will create a path of tracks from a start position until the next position by creating inclines between them
    //startTrack: track that this path is starting on
    public void CreateInclinePath(Vector3 position, GameObject startTrack) {
        //if any tracks should be created
        bool cancel = false;
        ...
        //calculate the slope for the target angle
        float targetSlopeAngle = 90 - targetAngle.x;
        float targetSlope = Mathf.Tan(targetSlopeAngle * Mathf.Deg2Rad);
        //calculate slope for the start
        float startSlopeAngle = 90 - startTrackAngleRelative.x;
        float startSlope = Mathf.Tan(startSlopeAngle * Mathf.Deg2Rad);

        //the b value for the target angle (b = y - mx)
        float targetB = targetPosition.y - targetSlope * targetPosition.z;
        //the b value for the start angle (b = y - mx)
        float startB = startPosition.y - startSlope * startPosition.z;
        ...
        //get distance from the start
        float distanceFromStart = Mathf.Sqrt(Mathf.Pow(collisionX - startPosition.z, 2)
            + Mathf.Pow(collisionY - startPosition.y, 2));

        //get distance from target
        float distanceFromTarget = Mathf.Sqrt(Mathf.Pow(collisionX - targetPosition.z, 2)
        ...
        //if the controller is on the bottom side
        bool bottomSide = targetPosition.z < startPosition.z;
        ...
        if (Mathf.Min(startTracksNeeded, targetTracksNeeded) == checkAmount && !cancel) {
            ...
            //y = rsinA, x = rcosA
            //these are the positions of these angles on a circle with a radius of 1
            float targetNormalX = Mathf.Cos((-targetAngle.x + 360) * Mathf.Deg2Rad);
            float targetNormalY = Mathf.Sin((-targetAngle.x + 360) * Mathf.Deg2Rad);
            float startNormalX = Mathf.Cos(startTrackAngleRelative.x * Mathf.Deg2Rad);
            float startNormalY = Mathf.Sin(startTrackAngleRelative.x * Mathf.Deg2Rad);

            //the radius would be equal to 1 for a circle like this. Find how much the distances between the points account for the radius of the circle
            float percentageOfRadius = Mathf.Sqrt(Mathf.Pow(startNormalX - targetNormalX, 2) + Mathf.Pow(startNormalY - targetNormalY, 2));

            //radius of the curve using the percentage calculations from above
            float radius = Mathf.Sqrt(Mathf.Pow(circleTargetX - startPosition.z, 2) + Mathf.Pow(circleTargetY - startPosition.y, 2)) / percentageOfRadius;

            //calculate the cirumference of this circle multiplied by the amount this curve takes up of the whole circle
            float curveLength = 2 * Mathf.PI * radius * (smallestAngleDifference.x / 360f);
            ...
            //find intersection between line to the start of curve from the end of curve
            float endToStartCurveSlope = Mathf.Tan((((180 - targetAngle.x) / 2)) * Mathf.Deg2Rad);
            //the b value (b = y - mx)
            float endToStartCurveB = targetPosition.y - endToStartCurveSlope * targetPosition.z;
            ...
            //radius of the curve using the percentage calculations from above
            float radius = Mathf.Sqrt(Mathf.Pow(circleStartX - targetPosition.z, 2) + Mathf.Pow(circleStartY - targetPosition.y, 2)) / percentageOfRadius;
        ...
        for (int i = 0; i < totalTracksNeeded(); i++) {
            ...
            if (startTrackAmount + i < trackPieces.Count || usePremadeTrackPiece) {
                ...
                if (!usePremadeTrackPiece) {
                    ...
                    //need to offset it by trackBoneSize by the angle
                    Vector3 offset = (new Vector3(0, -Mathf.Sin(eulerAngles.x * Mathf.Deg2Rad), Mathf.Cos(eulerAngles.x * Mathf.Deg2Rad)) * (trackBoneSize * 5f));
